...
-- Real, persistent cards for the homepage Browse grid. Replaces the old
-- hardcoded-in-HTML card data so stats like "Added by" and "Updated" can be
-- genuine instead of static placeholder text.
...
-- Seed the real cards that already exist on the site (safe to re-run:
-- ON CONFLICT means this won't duplicate or clobber edits made via the
-- admin UI after the first run).
